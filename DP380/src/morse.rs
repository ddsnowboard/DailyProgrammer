pub mod morse {
    ...
    fn find_order(s: &str, remaining: HashSet<char>) -> Option<String> {
        if remaining.is_empty() {
            return Some("".to_string());
        }
        for c in possible_first_letters(s, &remaining) {
            let new_hash_set: HashSet<char> =
                remaining.iter().filter(|&&x| x != c).map(|&c| c).collect();
            let new_string = consume(s, c);
            if let Some(mut s) = find_order(new_string, new_hash_set) {
                s.insert(0, c);
                return Some(s);
            }
        }
    ...
    pub fn smalpha(s: &str) -> Option<String> {
        find_order(s, ALPHABET_SET.clone())
    }
